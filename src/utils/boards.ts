...
export interface BoardNode {
  ...
  standalone?: boolean;
  /**
   * **上方那行位置链接（面包屑）跳去哪**。留空 = 点它就回它自己那一页。
   *
   * 子页面正文上方会有一行「花娅陌域 › 花娅编年史 › 纷湖 › 当前这页」，
   * 其中的祖先都是能点的。默认点「纷湖」进 `/huaya/years/fenhu`；
   * 填了这里，就从这一项点过去的时候改去这个地址
   * （hero 里那个「← 返回 纷湖」也一起跟着变 —— 两处标的是同一件事）。
   *
   * 允许 http(s) / mailto / tel / 站内 `/路径` / `#锚点`，和 `link` 同一套规则。
   * 用户 2026-09-22 的原话：「导航栏…上方会有可以点击的纷湖二字 如果预留了点击位
