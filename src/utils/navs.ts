...
export interface NavCategory {
  ...
  note?: string;
  /**
   * **这一块顶上那行里，点这个名字跳去哪**（可选）。
   *
   * 默认（不填）：跳到下面自己那一节（`#锚点`）—— 那是给「一块里挂了好几个分类」用的
   * 跳转索引；只有一个分类时那一节就在紧下面，点它等于什么都没发生。
   * 填了就变成真链接（站内 / 外链都认），外链会自动 `target="_blank"`。
   *
   * 用户 2026-09-22 的原话：「我说的是这个导航二字旁边的纷湖 为什么可以点 点了根本没反应
   * 要么加可以写链接的接口要么删掉」。
   */
  link?: string;
  groups: NavGroup[];
