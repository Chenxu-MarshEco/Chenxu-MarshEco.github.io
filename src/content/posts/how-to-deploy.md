...
1. 双击 `开始写作.cmd`，在浏览器里写文章、传图片
2. 想看效果就双击 `本地预览.cmd`，打开 `http://localhost:4321`
3. 满意了就双击 `发布到线上.cmd`，它会自动提交并推送
...
`发布到线上.cmd` 在推送前会先试着连一下 GitHub，连不上会直接告诉你，不会等 push 到一半才报一堆看不懂的错。
