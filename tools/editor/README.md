...
工具栏上的两个按钮打开两套图形化编辑，都不用碰 JSON：

**板块树**（`板块`）：管首页那两个大板块和它们下面的整棵树。

- 大板块自己的**名称 / 副标题 / 地址**能改（以前这三样是写死在数据里、界面上只能看）。
- 每个版块能改名、改地址、用 `＋下级` 往下加一层、删除（有下级会二次确认）。
- `移动到…` 把已有的版块换到别的大板块（或别的版块）下面。
- **封面图**：每个版块（含大板块）旁边都有缩略图和「封面图」按钮，
  选一张图就会传到 `public/img/uploads/`，路径写进该节点的 `image` 字段。
  页面上这张图会**铺满整个卡片**（微微半透明、名字压在正中）；
  没传的话那条卡片就是一条扁平的名字条，不摆空图位。
- **换父级不会改 id**：文章 frontmatter 的 `subs` 存的是 id，换了 id 归类就断了。
  但地址会跟着树变，除非那一项自己写了地址。
