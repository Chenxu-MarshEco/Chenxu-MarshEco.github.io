...
- **版式**：每一行（含大板块）都多一个下拉：**竖排**（默认，子页面一条条整行）
  或**三列分区**（左大块 + 中右两栏）。三列分区要该页有 4 个以上子版块才生效；
  这一页写了「页面内容」时以内容为准，版式不参与。
- **页面内容**：每个版块旁边的「页面内容」按钮打开一个小编辑器，
  可以往这一页里加八种块：**文字**（Markdown）、**图片**（可传图、选宽度档）、
  **链接**、**子页面**、**分隔线**、**两栏**、**视频**、**文章**。
  「子页面」块里除了整块的比例和大小，**每一张卡还能单独设**（空着就跟整块的）——
  那个设置写在子版块自己身上（`cardShape` / `cardSize`），换顺序、挪走都还跟着。
  块能上移下移；写了内容的页面就不再自动铺开子版块。
  每个块在「排版」模式里都能拖动缩放（锚点 `pg-<块id>`）。
  保存时会把整棵版块树一起写进 `home-boards.json`。
