...
上面那套是「子版块自动铺开」。想自己写，就用编辑器「板块」里每个版块旁边的
**「页面内容」**按钮 —— 一个版块页可以有自己的内容，四种块随便拼：
...
| **子页面** | 把这一层的子页面铺在**这里**，卡片的横竖比例（横 16:3 / 方 1:1 / 竖 3:4）和大小（大/中/小）都能选 |

块可以上移下移。**写了内容，这一页就不再自动铺开子版块** —— 子页面要靠
「子页面」块自己插到想要的位置。没写内容的页面一切照旧。

每个块在「排版」模式里都能拖动、缩放（锚点是 `pg-<块id>`）：选「大板块页」→
