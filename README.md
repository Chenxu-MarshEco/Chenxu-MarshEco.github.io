...
1. **选一张地图图**（走的是和图片块同一套上传 + 压缩）。
2. **在图上点一下就钉一个地标** —— 想在哪儿点哪儿，点错了就在下面清单里删掉。
3. 回到下面的**地标清单**填两样：**建筑名**（鼠标移上去显示的就是它）和
   **跳转地址**（写 `/huaya/xxx`。这一栏有整棵版块树的候选，敲两个字就补全；
   留空的话这个地标就只能看名字、点不动）。

页面上的效果：

- 地标是**蒸汽波粉色的塔吊**，格子条纹 + 下面一个小箭头，箭头尖指着图上那个点。
- **悬停**（或键盘 Tab 过去）浮出建筑名，**点击**跳到那一页。
- 图右边竖着三个按钮：**＋ / －** 缩放、**隐藏地标**（地标多了会盖住地图，一键收掉）。
- 地图可以**拖着看**，边缘拖不出框；滚轮要按住 **Ctrl / Cmd** 才缩放 ——
  不然鼠标从地图上划过就把页面滚动吞掉了。想回到全图点左下角的**复位**。

**位置存的是百分比**（相对图片宽高的 `0~100`），不是像素。所以放大、拖动、
换屏幕宽度、甚至换一张同比例的大图，地标都还钉在原来那个位置。
图钉本身做了反向缩放，地图放大 5 倍，图钉在屏幕上还是原来那么大。
