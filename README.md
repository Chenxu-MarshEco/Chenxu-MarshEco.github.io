...
**1 写文章**　打开可视化编辑器，在浏览器里写文章、传图片（`http://localhost:4322`）。
**2 看效果**　在浏览器里看站点真实效果（`http://localhost:4321`）。
**3 发布上线**　提交并推送到 GitHub，一两分钟后线上更新。
**4 首次设置**　**只跑一次**。检查环境、安装依赖。

选 1、2、3 之前如果依赖还没装，菜单会提示你先选 4。

> 第一次用之前，需要先装 **Node.js LTS**（<https://nodejs.org/>）和 **Git**（<https://git-scm.com/>），
> 都是一路点「下一步」。装完再双击 `花娅陌质流.cmd` 选 4。
...
菜单本身也能单独跑某一个动作，方便写成脚本：

```bash
node tools/menu.mjs setup      # 等同于菜单里的 4
node tools/menu.mjs publish    # 等同于菜单里的 3
```
...
双击 `花娅陌质流.cmd` 选 **1 写文章**，浏览器会打开 `http://localhost:4322`。
